...
function CreateAccessPointModal({
  onClose,
  onCreated,
}: {
  onClose: () => void;
  ...
  // Load locations for the active account.
  useEffect(() => {
  ...
  // Load devices when the chosen location changes.
  useEffect(() => {
    if (!locationId) {
      setDevices([]);
      return;
  ...
  return (
    <Modal open onClose={onClose} className="sm:max-w-lg">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <Field label="Name" value={name} onChange={setName} placeholder="e.g. Front Gate" />
